...
public class FractalExplorer {
    ...
    /** Класс для вычисления значений цвета для одной строки фрактала
       Создаем подкласс SwingWorker, который явл внутр классом FractalExplorer*/
    ...
    private class FractalWorker extends SwingWorker <Object, Object>
    {
        ...
        protected void done() {
            ...
            /**Используем метод, чтобы указать область перерисовки */
            display.repaint(0, 0, yCoordinate, displaySize, 1);
            /**Уменьшение значения «rows remaining» на 1, как последний шаг данной операции.
            Затем, если после уменьшения значение «rows remaining» равно 0, вызов метода
            enableUI (true) */
            rowsRemaining--;
            if (rowsRemaining == 0) {
                enableUI(true);
    ...
    /**ункция, которая включает или отключает кнопки интерфейса
    ...
    /**После в класс drawFractal добавляем метод enableUI(false) для отключеия кнопок во время рисования,
     * и "rows remaining", который устанавливается равным общему кол-ву строк
     */
    /**
